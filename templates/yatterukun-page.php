...
if ( is_page( $page_slug ) ) {
	if ( ! $_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if ( ! empty( $_FILES[ $data_name ]['name'] ) && $_FILES[ $data_name ][ 'size' ] > 0 ) {
			...
			if ( $_FILES[ $data_name ][ 'size' ] <= $max_size * 1024 * 1024 ) {
				...
				if ( is_array( $file_types ) && in_array($uploaded_file_ext, $file_types)
					...
					if ( 'jpg' == $uploaded_file_ext ) {
						...
						foreach ( glob ( ABSPATH .'wp-content/uploads/yatterukun/*.jpg' ) as $file ) {
							delete_files ( $file );
						}
						
						//
						$dst_file_origin = ABSPATH .'wp-content/uploads/yatterukun/yatterukun_origin.jpg';
						$dst_file_new = ABSPATH .'wp-content/uploads/yatterukun/yatterukun.jpg';
						...
						$dst_file = ABSPATH.'wp-content/uploads/yatterukun/yatterukun.mp4';
						if ( file_exists ( $dst_file ) ) {
							delete_files ( $dst_file );
						}
				...
				echo 'Cannot accept your file. Max file size is set to ';
				echo $max_size;
				echo ' MB.' . PHP_EOL;
			}
		}
